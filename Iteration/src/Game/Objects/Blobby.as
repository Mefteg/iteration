	...
	{				
		...
		override public function onClick():Boolean 
		{
			if ( FlxG.mouse.justPressed() )
			{
				...
				if ( Math.abs(MathUtils.clampAngle(m_pos) - clicAngle) < 2 && Math.abs(Point.distance(new Point(mouseX, mouseY), m_planet.center()) - (m_planet.radius()-95) * GameParams.map.zoom) < 8 )
				{
					return true;
				}
			}
