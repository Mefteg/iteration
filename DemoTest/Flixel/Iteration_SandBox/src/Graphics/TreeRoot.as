package Graphics 
{
	...
	public class TreeRoot extends FlxGroup 
	{
		...
		public function TreeRoot(origin:FlxPoint, baseColourR:uint, baseColourG:uint, baseColourB:uint, 
												  endColourR:uint,endColourG:uint,endColourB:uint, length:int) 
		{
			super();
			
			m_origin = new FlxPoint(origin.x,origin.y);
			m_actualPoint = new FlxPoint(origin.x,origin.y);
		...
		{
			// super.update();
			var distance:int = VectorUtils.length(m_actualPoint, m_origin);
			var interpolation:Number = distance / (m_lengthMax * m_lengthMax);
			if ( distance < m_lengthMax * m_lengthMax )
			{
